...
#===============================================================================
	...
	my $self = {
		sContext 							=> $sContext,		# the role or purpose of THIS object
...
# if all patterns captured, replaces all numeric order tags in given templates with corresponding captured values 
	...
	my $sEntryEdgeAttr; 
	my $sContextLabel; 	# for lines which may contain file name
...
#===============================================================================
	...
	my $sContextFileAsLabel 	=$self->getContextFileName();
	my $sLatestChainNodeName 	= $self->getLatestChainNode();
	my $sLatestChainNodeType 	= $self->getLatestChainNodeType();
	...
	$sFilledTemplate =~ s/_cluster_label_/$sContextFileAsLabel/g;
	$sFilledTemplate =~ s/_node_name_/$sTapNodeName/g;
	push(@asDotSyntaxLines, $sFilledTemplate);
...
#===============================================================================
	...
	my $sFilledTemplate;
	MM::TRACE_SUB($self->{sContext});
	my $sContextFileAsName =$self->getContextFileNameAsSym();
	my $sContextFileAsLabel =$self->getContextFileName();
	my @asDotSyntaxLines 	= ();
	my $sLatestChainNodeName 	= $self->getLatestChainNode();
	my $sLatestChainNodeType = $self->getLatestChainNodeType();
	...
	$sActivityNodeFullName = $sActivityNodeName."_in_".$sContextFileAsName;
	...
	$sFilledTemplate = $sCLUSTER_tpl;
	$sFilledTemplate =~ s/_cluster_name_/$sContextFileAsName/g;
	$sFilledTemplate =~ s/_cluster_label_/$sContextFileAsLabel/g;
